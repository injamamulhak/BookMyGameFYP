...
const ReviewItem = ({ review, currentUser, isOperator, onReviewDeleted, onReviewChanged }) => {
    ...
    return (
        <div className={`p-4 mb-4 rounded-xl border ${review.isFlagged ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100 shadow-sm'}`}>
            ...
            <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                        <h4 className="font-semibold text-gray-800">{review.user?.fullName}</h4>
                ...
                <div className="flex gap-2">
                    ...
                    {(isOperator || currentUser?.role === 'admin') && (
                        <button 
                            ...
                            title={review.isFlagged ? "Unflag review" : "Flag as inappropriate"}
                        >
                            <FaFlag size={14} />
                        </button>
                    )}
            ...
            {review.isFlagged ? (
                <div className="text-red-500 italic text-sm mt-2 mb-3 bg-red-100/50 p-2 rounded-lg">
                    This review has been flagged by the venue operator and is under moderation review.
                </div>
            ) : (
                <p className="text-gray-600 mt-2 mb-4 whitespace-pre-wrap">{review.comment}</p>
            )}

            {/* Actions */}
            <div className="flex items-center mt-2">
                {(isOperator || currentUser?.id === review.userId) && !review.isFlagged && (
                    <button 
            ...
            {review.replies && review.replies.length > 0 && (
                <div className="mt-4 ml-8 md:ml-12 flex flex-col gap-3">
                    {review.replies.map(reply => {
                        ...
                        const replyAuthorIsOperator = isOperator && currentUser.id === reply.userId;
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-2 flex-wrap text-sm">
                                            <span className="font-semibold text-gray-800">{reply.user.fullName}</span>
                                            {isOperatorReply && (
                                    ...
                                    <p className={`text-sm mt-1 text-gray-700 whitespace-pre-wrap ${isOperatorReply ? 'font-medium' : ''}`}>
                                        {reply.comment}
                                    </p>
                                </div>
                            </div>
