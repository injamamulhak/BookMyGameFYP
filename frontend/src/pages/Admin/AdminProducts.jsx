            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
                <div className="flex flex-col sm:flex-row gap-4">
                    <form onSubmit={handleSearch} className="flex-1 flex">
                        ...
                        />
                        <button type="submit" className="px-4 py-2 bg-primary-600 text-white rounded-r-lg hover:bg-primary-700 transition-colors text-sm">
                            Search
                        </button>
                    </form>
                    <select
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Seller</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {products.map((product) => (
                                    <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-sm font-medium text-gray-900">Rs. {parseFloat(product.price).toLocaleString()}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <button
                                                ...
                                                className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${product.isActive
                    ...
                    <div className="flex justify-center py-4 gap-2 border-t border-gray-100">
                        {[...Array(pagination.totalPages)].map((_, i) => (
                            <button
                                key={i + 1}
                                onClick={() => setPage(i + 1)}
                                className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${page === i + 1 ? 'bg-primary-600 text-white' : 'text-gray-600 hover:bg-gray-100'
                                    }`}
                            >
                                {i + 1}
                            </button>
