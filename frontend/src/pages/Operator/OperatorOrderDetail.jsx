...
function OperatorOrderDetail() {
  ...
  const [confirmModal, setConfirmModal] = useState({ open: false, newStatus: '' })
  ...
  const confirmUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await orderService.updateOperatorOrderStatus(
        id,
        newStatus,
      )
      if (response.data.success) {
  ...
  return (
    <div className='max-w-4xl mx-auto space-y-6'>
      <ConfirmModal
        ...
        title={`Mark Order as ${confirmModal.newStatus === 'completed' ? 'Completed' : 'Cancelled'}?`}
        message={`Are you sure you want to mark this order as ${confirmModal.newStatus}? This action will update the order status immediately.`}
        confirmText={confirmModal.newStatus === 'completed' ? 'Mark Completed' : 'Cancel Order'}
        confirmVariant={confirmModal.newStatus === 'completed' ? 'primary' : 'danger'}
        onConfirm={confirmUpdateStatus}
        onCancel={() => setConfirmModal({ open: false, newStatus: '' })}
      />
      {/* Header / Back Link */}
      ...
      <div className='bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden'>
        ...
        <div className='p-6 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
          ...
          <div className='flex items-center'>
            <span
              className={`inline-flex px-3 py-1 text-sm font-medium rounded-full border ${getStatusBadge(order.status)}`}
            >
        ...
        <div className='p-6 border-t border-gray-100'>
          ...
          <div className='space-y-4'>
            {order.items.map((item) => (
              <div
                key={item.id}
                className='flex items-center py-3 border-b border-gray-50 last:border-0 last:pb-0'
              >
